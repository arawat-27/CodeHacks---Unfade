...
function ProfileContent() {
  ...
  const removeProject = async (id) => {
    const reason = window.prompt("Why are you removing this project?");
    if (!reason?.trim()) return;
    const { error: removeError } = await supabase.rpc("withdraw_submission", {
      submission_id: id,
      reason: reason.trim(),
    });
    if (removeError) setError(removeError.message);
      ...
      setSubmissions((items) =>
        items.map((item) =>
          item.id === id
            ? { ...item, status: "removed", removal_reason: reason.trim() }
            : item
        )
      );
  };
  ...
  return (
    <div className="profile-page">
      ...
      <main className="profile-content">
        ...
        {loading && <p>Loading your work…</p>}
        {error && <p className="profile-error">{error}</p>}
        {!loading && !error && submissions.length === 0 && (
          <div className="empty-profile">
            <h2>No projects yet</h2>
            <p>Submit your first project from the Add Your Work page.</p>
          </div>
        )}
                ...
                <div>
                  ...
                  {submission.status !== "removed" && (
                    <button
                      ...
                      onClick={() => removeProject(submission.id)}
                    >
                      Remove project
                    </button>
                  )}
